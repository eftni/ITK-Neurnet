...
canvas gen_net(w_mat w, std::pair<int,int> neuron = {-1,-1}){
...
void draw(std::string fname/*, std::pair<int, int> neuron*/, int neuron = 0){
    w_mat w = load(fname);
    canvas actmap = gen_actmap(w, neuron);
    canvas net = gen_net(w, std::make_pair(0, neuron));
